...
const CDN_TAIL = '&w=1280&q=85';

const img = (file: string) => `${CDN}${file}${CDN_TAIL}`;
...
function ProjectCard({ project, index, totalCards }: ProjectCardProps) {
  ...
  return (
    <div ref={containerRef} className="flex h-[85vh] items-start justify-center">
      <motion.div
        className="sticky top-24 w-full rounded-[40px] border-2 border-[#D7E2EA] p-4 sm:rounded-[50px] sm:p-6 md:top-32 md:rounded-[60px] md:p-8"
        style={{
          scale,
          top: `${index * 28}px`,
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-4 px-2 pb-4 md:gap-8 md:pb-6">
